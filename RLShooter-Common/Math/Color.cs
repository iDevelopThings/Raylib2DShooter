...
public struct Color : IEquatable<Color> {
    ...
    public readonly int ToIntRGBA() {
        int result = 0;
        
        result = (int)(((uint)R << 24) |
        ((uint)G << 16) |
        ((uint)B << 8) |
        (uint)A);
    ...
    public static Color FromHex(uint hex) {
        var a = (float) (hex >> 24 & 0xff) / byte.MaxValue;
        var r = (float) (hex >> 16 & 0xff) / byte.MaxValue;
        var g = (float) (hex >> 8 & 0xff) / byte.MaxValue;
        var b = (float) (hex & 0xff) / byte.MaxValue;
        return new Color(r, g, b, a);
    }
    public static Color FromHex(string hex) {
        ...
        var color = uint.Parse(hex, System.Globalization.NumberStyles.HexNumber);
