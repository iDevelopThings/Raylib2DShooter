...
public static class ImageHelper
{
    ...
    /// <param name="image">The identifier of the image to display.</param>
    /// <param name="size">The size of the image.</param>
    public static void ImageCenteredH(ImTextureID image, Vector2 size, Color tintColor)
    {
        var windowWidth = ImGui.GetWindowSize().X;
        var imageWidth  = size.X;

        ImGui.SetCursorPosX((windowWidth - imageWidth) * 0.5f);
        Vector4 tint = new Vector4(tintColor.R, tintColor.G, tintColor.B, tintColor.A);
        ImGui.Image(image, size, tint);
    }
